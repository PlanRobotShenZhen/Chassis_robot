...
void data_task(void *pvParameters)
{
   ...
   while(1)
    {	
			vTaskDelayUntil(&lastWakeTime, F2T(RATE_1000_HZ));//此任务以20Hz的频率运行
			if (uart3_recv_flag)
			{
				uart3_recv_flag = 0;
...
void Usart5_Init(unsigned int unBound)
{
	...
	RCC_EnableAPB1PeriphClk(RCC_APB1_PERIPH_UART5, ENABLE);	//使能USART2，GPIOA时钟
    ...
    GPIO_InitPeripheral(GPIOC, &GPIO_InitStructure);//初始化GPIOC.12
   
  //USART5_RX	  GPIOD.2初始化
    GPIO_InitStructure.Pin = GPIO_PIN_2;//PD2
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//浮空输入
    GPIO_InitPeripheral(GPIOD, &GPIO_InitStructure);//初始化GPIOD.2  
  ...
  //USART2 NVIC 配置
   ...
   //USART 初始化设置
	...
	USART_InitStructure.Parity = USART_PE_EVEN;//无奇偶校验位
	USART_InitStructure.HardwareFlowControl = USART_HFCTRL_NONE;//无硬件数据流控制
	USART_InitStructure.Mode = USART_MODE_RX;	//收发模式

    USART_Init(UART5, &USART_InitStructure); //初始化串口5
    USART_ConfigInt(UART5, USART_INT_RXDNE, ENABLE);//开启串口接受中断
...
void uart3_dma_config(void)
{
	...
	DMA_DeInit(DMA1_CH2);  // DMA1 通道2, UART3_TX
	DMA_DeInit(DMA1_CH3);  // DMA1 通道3, UART3_RX

	RCC_EnableAHBPeriphClk(RCC_AHB_PERIPH_DMA1, ENABLE);  // 使能DMA1的时钟

	// 配置 DMA1 通道2, UART3_TX
	DMA_InitStruct.PeriphAddr = (USART3_BASE + 0x04);   // 数据寄存器(USART_DR) 地址偏移：0x04
	...
	USART_EnableDMA(USART3, USART_DMAREQ_TX | USART_DMAREQ_RX, ENABLE);// 使能DMA串口发送和接受请求

	DMA1_Channel2_nvic_config();

	// 配置 DMA1 通道2, UART3_TX 传输完成中断
	DMA_ConfigInt(DMA1_CH2, DMA_INT_TXC, ENABLE);

	DMA_EnableChannel(DMA1_CH2, ENABLE);     // 开启接收
	DMA_EnableChannel(DMA1_CH3, DISABLE);    // 禁止发送
}
...
void Usart3_init(u32 bound)
{
	...
	RCC_EnableAPB2PeriphClk(RCC_APB2_PERIPH_AFIO,ENABLE);// 需要使能AFIO时钟
	RCC_EnableAPB2PeriphClk(RCC_APB2_PERIPH_GPIOC, ENABLE);	//使能GPIO时钟
    RCC_EnableAPB1PeriphClk(RCC_APB1_PERIPH_USART3, ENABLE);	//使能USART3时钟
	GPIO_ConfigPinRemap(GPIO_PART_RMP_USART3, ENABLE);//引脚重映射
	//USART_TX  
    ...
    USART_ConfigInt(USART3, USART_INT_IDLEF, ENABLE);//开启串口接受中断
    USART_Enable(USART3, ENABLE);                    //使能串口3 
}
...
{	
	if(USART_GetIntStatus(UART5,USART_INT_RXDNE) != RESET) //中断产生 
	{
		g_eControl_Mode = CONTROL_MODE_REMOTE; //航模遥控方式选择
		USART_ClrIntPendingBit(UART5,USART_INT_RXDNE); //清除中断标志			 
		Uart5_Buffer[ucRcvCount++] = USART_ReceiveData(UART5);     //接收串口1数据到buff缓冲区
		...
		{
			ucRcvCount = 0;                                   //不是我们需要的数据或者达到最大接收数则开始重新接收
		}
	}
}
...
void DMA1_Channel2_IRQHandler(void)
{
	if (DMA_GetIntStatus(DMA1_INT_TXC2, DMA1) != RESET) // DMA1 通道2, UART3_TX 传输完成
	{
		...
		uart3_send_flag = 1;
		DMA_EnableChannel(DMA1_CH2, DISABLE);        // 关闭 DMA1 通道4, UART1_TX
...
void USART3_SEND(void)
{
	...
	DMA_SetCurrDataCounter(DMA1_CH2, uart3_send_len);  // 传输数量寄存器只能在通道不工作(DMA_CCRx的EN=0)时写入
	DMA_EnableChannel(DMA1_CH2, ENABLE);    // 开启 DMA1 通道2, UART3_TX	
}
