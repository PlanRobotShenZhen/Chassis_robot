...
void RCC_HSE_Config(unsigned int unDiv, unsigned int unPllm)
{
	 ...
	 RCC_ConfigHse(RCC_HSE_ENABLE); //设置外部高速晶振(HSE)
	 if(RCC_WaitHseStable() == SUCCESS) //等待HSE起振
	 {
			RCC_ConfigHclk(RCC_SYSCLK_DIV1); //设置AHB(HCLK)时钟
			RCC_ConfigPclk1(RCC_HCLK_DIV2); //设置APB1低速时钟(PCLK1)
			RCC_ConfigPclk2(RCC_HCLK_DIV1); //设置APB2高速时钟(PCLK2)
		 //配置PLL时钟源(u32Div)以及其倍频系数(u32Pill),
		 //来自外部高速时钟的几分频,第二个参数为PLL输出时钟及倍频,
		 //默认是9,即为72MHz?
			RCC_ConfigPll(unDiv,unPllm);
			RCC_EnablePll(ENABLE);//使能或者失能PLL
			while(RCC_GetFlagStatus(RCC_FLAG_PLLRD) == RESET); //检查指定的RCC标志位设置与否, PLL就绪
			RCC_ConfigSysclk(RCC_SYSCLK_SRC_PLLCLK); //设置系统时钟(SYSCLK)
			while(RCC_GetSysclkSrc()!=0x08);//返回用作系统时钟的时钟源,0x08:PLL作为系统时钟
	 }
}
...
void systemInit(void)
{
	...
	RCC_HSE_Config(RCC_PLL_SRC_HSE_DIV1,RCC_PLL_MUL_9);
