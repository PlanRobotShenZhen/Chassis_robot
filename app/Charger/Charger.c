...
{	
	...
	if (NoSignalCout >= 200){// 检查无信号计数器是否达到阈值
		pdu[BatteryTemperature] = 3;//连续两秒没接收到信号，则返回充电异常信号
		return 3;
	}
	if (BitEnd == 1){// 如果接收函数启动标志被设置
...
{
    if (TIM_GetIntStatus(MCU_INF_RX_TIM, TIM_INT_CC2) == SET) {// 检查 TIM5 捕获/比较通道 2 的中断状态
...
{
    if (GPIO_ReadInputDataBit(KEY_GPIO, KEY_PIN) == SET) {// 检查按键是否被按下
        while (GPIO_ReadInputDataBit(KEY_GPIO, KEY_PIN) == SET);// 等待按键被释放
        RGB_ChangeFlag = 1;// 设置 RGB 灯光变化标志 
        RGB_ArrayNum++;// 增加 RGB 灯光模式数组索引
        if (RGB_ArrayNum >= (sizeof(RGB_Array) / sizeof(RGB_Array[0]))) {// 如果索引超过 RGB 灯光模式数组的长度，则循环到开始
            RGB_ArrayNum = 0;
        }
    }
}
	...
	if (TIM_GetIntStatus(RGB_TIM, TIM_INT_UPDATE) == SET){
		...
		if (RGB_ChangeFlag == 1){
			...
			if (Reset_i > 0){
				...
				Reset_i--;
				Reset_i--;
			}else if (RGB_chips > 0){//逐次发送每颗芯片的rgb数据
