...
bool Sbus_Data_Parsing_Flag = false;	//航模数据处理标志位
...
/*共25个字节，每个字节包括8位数据位，2位停止位，偶效验。
起始字节为0F，结束字节00，第2个字节~第23个字节为对应16个通道，每个通道11bit。22*8=16*11，每个通道取值范围为0~2^11-1=0-2047，本款取值240-1807。
ch1的11位=data2的低3位+data1的8位，依此类推。第24个字节为状态字flag，若为0，则所有正常。
*/
	if(REMOTE_Count < 10000)	REMOTE_Count++;
	if(Sbus_Data_Parsing_Flag){
		if(Uart5_Buffer[23] == 0){
...
void MotorEnableFunction()
{
	for(uint16_t count = 0; count < pdu[motor_number]; count++){
		uint16_t x_sw = motor1_state_word + count * pdu[ro_motor_gap];
		uint16_t x_enstate = motor1_enable_state + count * pdu[ro_motor_gap];
		if ((Abs_int(pdu[rc_ch7_value] - pdu[rc_max_value]) < CHANNEL_VALUE_ERROR) ||
		(robot_control.bit.motor_en)){//开关处于打开状态
			if(!pdu[x_enstate]){
				switch (pdu[x_sw] & 0x0F){
					case 0x00:	mrd[count].d.ctrl.cw = 0x06; 	pdu[x_enstate] = disable_state;		break;
					case 0x01:	mrd[count].d.ctrl.cw = 0x07; 	pdu[x_enstate] = disable_state;		break;
					case 0x03:	mrd[count].d.ctrl.cw = 0x0F; 	pdu[x_enstate] = disable_state;		break;
					case 0x07:	Motor_Enable_Flag = true;		pdu[x_enstate] = enable_state;		break;													
				}
			}
		}else if((pdu[rc_ch7_value] - pdu[rc_min_value]) < CHANNEL_VALUE_ERROR){
			if(pdu[x_enstate]){
				switch (pdu[x_sw] & 0x0F){
					case 0x07:	mrd[count].d.ctrl.cw = 0x07;	pdu[x_enstate] = enable_state;		break;
					case 0x03:	mrd[count].d.ctrl.cw = 0x06;	pdu[x_enstate] = enable_state;		break;
					case 0x01:	mrd[count].d.ctrl.cw = 0x00;	pdu[x_enstate] = enable_state;		break;
					case 0x00:	Motor_Enable_Flag = false;		pdu[x_enstate] = disable_state;		break;																	
				}			
			}
		}
	}
